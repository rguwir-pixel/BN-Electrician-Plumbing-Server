...
const messages = [];
const subscriptions = [];
const otpStore = new Map(); // नया: OTP सेव करने के लिए

// Nodemailer सेटअप (ईमेल भेजने के लिए)
...
// 1. नया API: OTP रिक्वेस्ट करने के लिए
app.post("/api/request-otp", async (req, res) => {
  ...
  otpStore.set(normalized, { otp, expires: Date.now() + 300000 }); // 5 मिनट के लिए

  try {
    await transporter.sendMail({
      from: `"BN Electrician" <${process.env.EMAIL_USER}>`,
      to: normalized,
      subject: "आपका लॉगिन OTP",
      text: `BN Electrician & Plumbing में लॉगिन करने के लिए आपका OTP है: ${otp}\nयह 5 मिनट के लिए मान्य है।`
    });
...
// 2. अपडेटेड API: OTP के ज़रिए लॉगिन करने के लिए
app.post("/api/login",(req,res)=>{
  ...
  if (!storedData) return res.status(400).json({ error: "OTP not found. Request again." });
  ...
  otpStore.delete(normalized); // सही होने पर OTP डिलीट करें
  ...
  if(!user){
    ...
    user.token = crypto.randomBytes(32).toString("hex"); // सुरक्षा के लिए नया टोकन
  }
